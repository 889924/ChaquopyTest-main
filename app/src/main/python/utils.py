...
def random_batch(speaker_num=config.N, utter_num=config.M, shuffle=True, noise_filenum=None, utter_start=0):
    ...
    if config.train:
        path = join(dirname(__file__),config.train_path)
    else:
        path = join(dirname(__file__),config.test_path)
    ...
    if config.tdsv:
        ...
        '''
        np_file_list = os.listdir(join(dirname(__file__),path))
        # ===========================================================2020/05/22
        if config.train == True:
            # 修改说话人个数
            np_file_list = np_file_list[:config.N]
        # ===========================================================结束
        ...
        for k in range(config.N):
            ...
            utters = np.load(os.path.join(path, file))  #  os.path.join(path, file)load utterance spectrogram of selected speaker
            # print('----',os.path.join(path, file))
            # print('-----------------',utters)
            if shuffle:
                utter_index = np.random.randint(0, utters.shape[0], utter_num)  # select M utterances per speaker
                utter_batch.append(np.concatenate(utters[utter_index], axis=1))
            else:
                if k == config.N-1:
                    utter_batch.append(np.concatenate([utters[0]]*utter_num, axis=1))
                else:
                    utter_batch.append(np.concatenate(utters[utter_start: utter_start + utter_num], axis=1))  # ====================2020/04/01 21:42

        utter_batch = np.concatenate(utter_batch, axis=1)
        utter_batch = np.abs(utter_batch) ** 2
        mel_basis = librosa.filters.mel(sr=config.sr, n_fft=config.nfft, n_mels=40)
        utter_batch = np.log10(np.dot(mel_basis, utter_batch) + 1e-6)  # log mel spectrogram of utterances
